...
var $tmpl_1$1 = template("<p>Child Component</p><button> </button>");
function ChildComponent() {
	...
	const dispatchCount = () => {
		this.dispatchEvent(new CustomEvent("child-count-changed", {
			bubbles: true,
			detail: { count: count.v }
		}));
	};
	this.connected(() => {
		const $root_1 = $tmpl_1$1();
		this.appendChild($root_1);
		const button_1 = this.firstChild.nextSibling;
...
delegate(["click"]);
var $tmpl_1 = template("<p>Parent Component</p><input type=\"number\"><button>Set child count</button><child-component></child-component><p class=\"child-message\"> </p>");
function ParentComponent() {
	...
	const passDataIntoChild = () => {
		this.querySelector("child-component").setCount(count.v);
	};
	this.on("child-count-changed", (event) => {
		childCount.v = event.detail.count;
		childCount.ref_1.nodeValue = "Count from child: " + childCount.v;
	});
	this.connected(() => {
		const $root_1 = $tmpl_1();
		this.appendChild($root_1);
		const input_1 = this.firstChild.nextSibling;
		const button_1 = input_1.nextSibling;
		const p_2_text = button_1.nextSibling.nextSibling.firstChild;
		input_1.__input = (e) => {
			count.v = Number(e.target.value);
			count.ref_1.value = count.v;
		};
		button_1.__click = passDataIntoChild;
		input_1.value = count.v;
		count.ref_1 = input_1;
		p_2_text.nodeValue = "Count from child: " + childCount.v;
		childCount.ref_1 = p_2_text;
	});
}
