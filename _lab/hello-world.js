...
var $tmpl_1 = template("<label for=\"name\">Enter name:</label><input id=\"name\" type=\"text\"><p id=\"msg\"> </p>");
function HelloWorld() {
	const name = {
		v: "World",
		e: []
	...
	const updateName = (event) => {
		name.v = event.target.value;
		name.ref_1.value = name.v;
		name.ref_2.nodeValue = "Hello " + name.v + "!";
	};
	this.connected(() => {
		const $root_1 = $tmpl_1();
		...
		input_1.__input = updateName;
		input_1.value = name.v;
		name.ref_1 = input_1;
		p_1_text.nodeValue = "Hello " + name.v + "!";
		name.ref_2 = p_1_text;
	});
}
