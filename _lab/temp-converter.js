...
var $tmpl_1 = template('<input type="number">°C =<input type="number">°F');
function TempConverter() {
	...
	const setFromC = (e) => {
		...
		celsius.v = c;
		celsius.ref_1.value = celsius.v;
		fahrenheit.v = ((c * 9) / 5 + 32).toFixed(1);
		fahrenheit.ref_1.value = fahrenheit.v;
	};
	const setFromF = (e) => {
		const f = parseFloat(e.target.value) || 0;
		fahrenheit.v = f;
		fahrenheit.ref_1.value = fahrenheit.v;
		celsius.v = (((f - 32) * 5) / 9).toFixed(1);
		celsius.ref_1.value = celsius.v;
	};
	this.connected(() => {
		const $root_1 = $tmpl_1();
		...
		input_1.__input = setFromC;
		input_2.__input = setFromF;
		input_1.value = celsius.v;
		celsius.ref_1 = input_1;
		input_2.value = fahrenheit.v;
		fahrenheit.ref_1 = input_2;
	});
}
