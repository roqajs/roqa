...
function App() {
	...
	const addNumber = () => {
		...
		numbers.ref_1.nodeValue = numbers.v.join(" + ") + " = " + sum.v;
		sum.v = numbers.v.reduce((a, b) => a + b, 0);
		sum.ref_1.nodeValue = numbers.v.join(" + ") + " = " + sum.v;
	};
	this.connected(() => {
		const $root_1 = $tmpl_1();
		...
		button_1.__click = addNumber;
		p_1_text.nodeValue = numbers.v.join(" + ") + " = " + sum.v;
		numbers.ref_1 = p_1_text;
		p_1_text.nodeValue = numbers.v.join(" + ") + " = " + sum.v;
		sum.ref_1 = p_1_text;
	});
}
