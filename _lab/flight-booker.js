...
var $tmpl_1 = template(
	'<input type="text" placeholder="Destination"><select><option value="one-way">One-way</option><option value="round-trip">Round-trip</option></select><input type="date"><input type="date"><button>Book</button><p>Return date must be after departure date.</p><p>Please choose a destination.</p>',
);
...
function FlightBooker() {
	...
	const bookTrip = () => {
		const message = isRoundTrip()
			? `You booked a round-trip to ${destination.v} leaving on ${departDate.v} and returning on ${returnDate.v}.`
			: `You booked a one-way flight to ${destination.v} leaving on ${departDate.v}.`;
		alert(message);
	};
	this.connected(() => {
		const $root_1 = $tmpl_1();
