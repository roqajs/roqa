...
function App(this: RoqaElement) {
	...
	const getScales = () => {
		...
		const yScale = d3
			.scaleLinear()
			.domain([0, d3.max(currentData, (d: { label: string; value: number }) => d.value) || 100])
			.nice()
			.range([innerHeight, 0]);
	...
	this.connected(() => {
		const xAxisGroup = this.querySelector(".x-axis") as SVGGElement;
		const yAxisGroup = this.querySelector(".y-axis") as SVGGElement;
