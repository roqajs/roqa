...
export interface CommentPageMethods {
...
function CommentPage(this: RoqaElement<CommentPageMethods>) {
	...
	const fetchComments = async (kids: number[]) => {
	...
	const doLoad = async (storyId: number) => {
		set(loading, true);
		set(error, false);
		set(story, null);
		set(comments, []);
	...
	return (
		<section class="comment-page">
			...
			<Show when={!get(loading) && !get(error) && get(story) !== null}>
				<div class="content">
					<button class="back-btn" onclick={() => this.emit("back")}>
						← Back to stories
					</button>
