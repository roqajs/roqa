 ...
 * Rift's reactive primitives directly into the generated code:
 *
 *   get(cell)       -> cell.v
 ...
 *
 * This optimization is critical for Rift's zero-overhead reactive model.
 */
...
`;
	...
	describe("import cleanup", () => {
		it("removes inlined imports from rift-js", () => {
			const code = `import { cell, get, set } from "rift-js";
const count = cell(0);
const x = get(count);
set(count, 1);`;
			const result = inlineGetCalls(code, "test.js");
			// Imports should be cleaned up (cell, get, set removed)
			expect(result.code).not.toMatch(/import.*\bcell\b.*from "rift-js"/);
			expect(result.code).not.toMatch(/import.*\bget\b.*from "rift-js"/);
			expect(result.code).not.toMatch(/import.*\bset\b.*from "rift-js"/);
		});

		it("preserves non-inlined imports", () => {
			const code = `import { cell, get, defineComponent, template } from "rift-js";
const count = cell(0);
const x = get(count);`;
			const result = inlineGetCalls(code, "test.js");
		...
		it("removes put import when inlined", () => {
			const code = `import { cell, put } from "rift-js";
const count = cell(0);
put(count, 5);`;
			const result = inlineGetCalls(code, "test.js");
			expect(result.code).not.toMatch(/import.*\bput\b.*from "rift-js"/);
			expect(result.code).toContain("count.v = 5");
		});

		it("handles empty import after all inlined", () => {
			const code = `import { cell, get } from "rift-js";
const x = cell(0);
const y = get(x);`;
			const result = inlineGetCalls(code, "test.js");
