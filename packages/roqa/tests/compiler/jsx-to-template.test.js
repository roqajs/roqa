...
describe("extractTemplate", () => {
	...
	it("handles Roqa custom elements with prop bindings", () => {
		const ast = parse("<my-counter value={get(count)}></my-counter>", "test.jsx");
		const jsxNode = ast.program.body[0].expression;
		const registry = new TemplateRegistry();
		const nameGen = new VariableNameGenerator();
		// Pass my-counter as a Roqa component tag
		const roqaTags = new Set(["my-counter"]);
		const result = extractTemplate(jsxNode, registry, nameGen, false, false, roqaTags);
	...
	it("handles third-party web components with attribute bindings", () => {
		...
		const registry = new TemplateRegistry();
		const nameGen = new VariableNameGenerator();
		// Not in Roqa tags - should be treated as third-party
		const roqaTags = new Set(["my-counter"]);
		const result = extractTemplate(jsxNode, registry, nameGen, false, false, roqaTags);
