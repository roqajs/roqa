...
// MIR schema types are also exposed via `roqa/ir`. Prefer that path for
// frontend contracts and authoring tools; use `roqa/compiler` for executable
// compiler APIs like `compile()` and `compileExpr()`.

export type ExprContext = {
	/** Inside an inline event handler. */
	isInlineHandler?: boolean;
	/** Component function name for generated closures. */
	componentName?: string;
	/** Collection name -> key field. */
	collectionKeys?: Map<string, string>;
};

export function compile(mir: ComponentIR | ComponentIR[]): CompileResult;
export function compileExpr(expr: ExprIR, ctx?: ExprContext): string;
